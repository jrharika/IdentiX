...
contract IdentX {
    ...
    function join_as_client(string identifying_info, uint256 date_of_birth, Sex sex, Gender gender, ID_Type id_type) external {
        ...
        Client memory new_client = Client({identifying_info: identifying_info, date_of_birth: date_of_birth, sex: sex, gender: gender, id_type: id_type, identifying_info_for_inst: "", inst_checked_in_at: 0, exists: true, record_metas: new Record_Meta[](0)});
    ...
    function add_record(address client_pub_addr, string permission_key_hash, uint256 time_of_visit, string ipfs_address) external {
        ...
        Record_Meta memory new_record = Record_Meta({inst_pub_addr: msg.sender, permission_key_hash: permission_key_hash, time_of_visit: time_of_visit, ipfs_address: ipfs_address});
        client_map[client_pub_addr].record_metas.push(new_record);
    }
